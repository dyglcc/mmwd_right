...
public class ShareGuideView extends RelativeLayout {
	...
	@SuppressWarnings("deprecation") public static void showAddGuide(MainActivity activity, View view) {
		...
		win.setContentView(ShareGuideView_.build(activity).setData(activity));
		win.showAsDropDown(view, 0, - QFCommonUtils.getScreenHeight(activity));
	}
	
	@ViewById ImageView iv_share, star1, star2, star3;
	...
	public ShareGuideView setData(MainActivity activity) {
	...
	@Click void iv_share(View view) {
		activity.onShareGuide(view);
		if (win != null && win.isShowing()) {
			win.dismiss();
		}
	}
